...
public class ChangePlayerIcon : MonoBehaviour
{
    ...
    // ★ iconChanger → 複数登録可能な List に変更
    [SerializeField] List<IconChanger> iconChangers = new List<IconChanger>();
    ...
    private void OnEnable()
    {
        ...
        foreach (var playerIcon in ownedIcons)
        {
            ...
            if (playerIcon.style == PlayerIconStyle.Chara)
                icon = Instantiate(playerIconPrefab, changePlayerIconScrollViewBase);
            else if (playerIcon.style == PlayerIconStyle.BackGround)
                icon = Instantiate(playerIconPrefab, changePlayerIconBGScrollViewBase);
    ...
    async void ChangeIcon(PlayerIconData _playerIcon)
    {
        ...
        // ★ ここで複数 iconChanger に反映！
        foreach (var changer in iconChangers)
        {
            if (changer != null)
                changer.UpdatePlayerIcon();
        }
